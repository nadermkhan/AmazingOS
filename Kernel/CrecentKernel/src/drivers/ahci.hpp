...
struct HBAPort {
    ...
    volatile uint32_t sctl;      // 0x2C, SATA control (SCR1:SControl)
    volatile uint32_t serr;      // 0x30, SATA error (SCR2:SError)
...
struct HBAMemory {
    ...
    volatile uint8_t  vendor[96];// 0xA0 ~ 0xFF, Vendor specific registers
    HBAPort  ports[32]; // 0x100 ~ 0x10FF, Port control registers
} __attribute__((packed));
...
struct AHCICommandHeader {
    ...
    uint8_t  r:1;       // Reset
    uint8_t  c:1;       // BIST
    uint8_t  c_clear:1; // Clear busy on OK
...
struct AHCIPrdtEntry {
    ...
    uint32_t dbc:22;    // Byte count, 4MB max
...
struct AHCICommandTable {
    ...
    AHCIPrdtEntry prdt_entry[1]; // PRDT entries
} __attribute__((packed));
...
struct FIS_REG_H2D {
    ...
    uint8_t  countl;    // Count register low (7:0)
    uint8_t  counth;    // Count register high (15:8)
